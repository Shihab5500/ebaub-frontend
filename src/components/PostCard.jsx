...
const PostCard = ({ post }) => {
  ...
  // 🔥 NEW: Save Edited Comment Function
  const handleSaveEditedComment = async (commentId) => {
    if (!editedCommentText.trim()) return toast.error("Comment cannot be empty!");
    
    try {
        const res = await fetch(`${API_URL}/api/posts/${safePost._id}/comments/${commentId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ userId: dbUser._id, text: editedCommentText })
        });
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-white rounded-2xl shadow-md p-5 mb-6 border border-gray-100 relative w-full max-w-full overflow-hidden">
      ...
      {/* 🔥 PRESERVED ACTIONS LAYOUT (Mobile: Flex, Desktop: Grid) */}
      <div className="
        border-t pt-3 text-gray-600 relative select-none w-full
        flex justify-between items-center                /* Mobile Style: Flex row */
        lg:grid lg:grid-cols-2 lg:gap-3 lg:items-center  /* Desktop Style: Grid 2 columns */
            ...
            {commentsArray.map((c, i) => (
              <div key={i} className="flex gap-2 items-start group">
                ...
                <div className="flex-1 min-w-0"> 
                  ...
                  {/* 🔥 UPDATED: Comment Display vs Edit Mode */}
                  {editingCommentId === c._id ? (
                      <div className="w-full">
                          <textarea 
                              value={editedCommentText} 
                              onChange={(e) => setEditedCommentText(e.target.value)} 
                              className="w-full border p-2 rounded-lg text-sm focus:border-primary outline-none bg-gray-50"
                          />
                          <div className="flex gap-2 mt-1">
                              <button onClick={() => handleSaveEditedComment(c._id)} className="text-xs bg-green-600 text-white px-2 py-1 rounded hover:bg-green-700 font-bold">Save</button>
                              <button onClick={() => setEditingCommentId(null)} className="text-xs bg-gray-400 text-white px-2 py-1 rounded hover:bg-gray-500 font-bold">Cancel</button>
                          </div>
                      </div>
                  ) : (
                      <div className="bg-white p-2 rounded-2xl text-sm shadow-sm border inline-block pr-4 relative max-w-full">
                  ...
                  <div className="text-[10px] text-gray-400 ml-2 mt-1 flex items-center gap-3">
                    ...
                    {/* 🔥 EDIT & DELETE ACTIONS */}
                    {dbUser && (dbUser._id === c.user?._id) && (
                         <>
                            {/* Edit Button for Comment Owner */}
                            <button onClick={() => { setEditingCommentId(c._id); setEditedCommentText(c.text); }} className="text-blue-500 hover:text-blue-700 font-bold cursor-pointer hover:underline">Edit</button>
                    ...
                    {/* Admin/Moderator can only delete */}
                    {dbUser && (dbUser.role === 'admin' || dbUser.role === 'moderator') && dbUser._id !== c.user?._id && (
                        <button onClick={() => handleDeleteComment(c._id)} className="text-red-400 hover:text-red-600 font-bold cursor-pointer hover:underline">Delete</button>
                    )}
