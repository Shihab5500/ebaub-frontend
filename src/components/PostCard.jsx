...
//     try {
//       const res = await fetch(`https://ebaub-backend.vercel.app/api/posts/${safePost._id}/react`, {
//         method: 'PUT',
...
//       try {
//         const res = await fetch(`https://ebaub-backend.vercel.app/api/posts/${safePost._id}/comments/${commentId}`, {
//           method: 'DELETE',
//           headers: { 'Content-Type': 'application/json' },
...
//     if (result.isConfirmed) {
//       try {
//         await fetch(`https://ebaub-backend.vercel.app/api/posts/${safePost._id}`, { method: 'DELETE' });
//         setCurrentPost(null);
//         Swal.fire({
//             title: "Deleted!",
//             icon: "success",
...
//     try {
//       const res = await fetch(`https://ebaub-backend.vercel.app/api/posts/${safePost._id}`, {
//         method: 'PUT',
//         headers: { 'Content-Type': 'application/json' },
...
//     try {
//       const res = await fetch(`https://ebaub-backend.vercel.app/api/posts/${safePost._id}/comment`, {
//         method: 'POST',
//         headers: { 'Content-Type': 'application/json' },
...
const PostCard = ({ post }) => {
  ...
  // --- Handlers --- (Same logic, URLs updated)
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-white rounded-2xl shadow-md p-5 mb-6 border border-gray-100 relative w-full max-w-full overflow-hidden">
      ...
      {images && images.length > 0 ? (
        <div className={`grid gap-1 mb-4 rounded-xl overflow-hidden border bg-gray-50
          ${images.length === 1 ? 'grid-cols-1' : ''}
          ${images.length === 2 ? 'grid-cols-2' : ''}
          ${images.length >= 3 ? 'grid-cols-2' : ''}
        `}>
          {images.map((imgUrl, index) => (
            <div key={index} className={`relative overflow-hidden ${images.length >= 3 && index === 0 ? 'col-span-2 row-span-2' : 'h-48'}`}>
              <img 
        ...
        // Backward compatibility for old single image posts
        <div className="w-full h-auto max-h-96 overflow-hidden rounded-xl mb-4 bg-gray-50 border">
            <img src={image.replace('/upload/', '/upload/w_800,q_auto,f_auto/')} alt="Post" className="w-full h-full object-contain" loading="lazy"/>
        </div>
      )}
